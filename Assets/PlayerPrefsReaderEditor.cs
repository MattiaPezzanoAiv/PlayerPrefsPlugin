namespace MP.Unity.Editor
{
    ...
    public class PlayerPrefsReaderEditor : EditorWindow
    {
        ...
        public static void FillPrefs()
        {
            for (int i = 0; i < 10; i++)
            {
                PlayerPrefs.SetFloat("pref_test_float" + i, i + 0.5f);
                PlayerPrefs.SetInt("pref_test_int" + i, i);
                PlayerPrefs.SetString("pref_test_str" + i, "test" + i);
            }
        }

        [MenuItem("PlayerPrefs/Show Window")]
        ...
        private void Left()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            if (GUILayout.Button(EditorGUIUtility.IconContent("TreeEditor.Refresh"), leftLayoutOption.ToArray()))
            {
                reader.Refresh();
                GUI.FocusControl(null);
            }
